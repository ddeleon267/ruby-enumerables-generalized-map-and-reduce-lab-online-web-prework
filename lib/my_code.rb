...
def reduce(arr, start = nil) # get them set up with correct parameters first
  ...
  if start
    ...
    running_total = arr[0] # if a start is not given, the starting point is first item in the array
    i = 1 # so you start at the second item when adding
  end
  while i < arr.length
    running_total = yield(running_total, arr[i]) # you are yielding to a block that contains a rule, but you need to provide starting point and next item to add
    i += 1
  end
  running_total
end